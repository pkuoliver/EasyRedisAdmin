...
if (isset($_POST['type'], $_POST['key'], $_POST['value'])) {
	...
	if ($_POST['type'] == 'string') {
		$redis->set($key, $value);
	} else if (($_POST['type'] == 'hash') && isset($_POST['hkey'])) {// Hash
	...
	} else if (($_POST['type'] == 'list') && isset($_POST['index'])) {// List
		$size = $redis->lLen($key);

		if (($_POST['index'] == '') || ($_POST['index'] == $size)) {
			// Push it at the end
	...
	} else if (($_POST['type'] == 'zset') && isset($_POST['score'])) {// ZSet
...
if ($edit) {
	...
	} else if (($_GET['type'] == 'hash') && isset($_GET['hkey'])) {// Hash
		$value = $redis->hGet($_GET['key'], $_GET['hkey']);
	} else if (($_GET['type'] == 'list') && isset($_GET['index'])) {// List
		$value = $redis->lIndex($_GET['key'], $_GET['index']);
	} else if ((($_GET['type'] == 'set') || ($_GET['type'] == 'zset')) && isset($_GET['value'])) {// Set, ZSet
		$value = $_GET['value'];
	}
